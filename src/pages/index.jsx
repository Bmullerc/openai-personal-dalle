...
export default function Home() {
  ...
  return (
    <>
      ...
      <main className="h-screen bg-zinc-200 pb-20">
        ...
        <div className="grid grid-flow-col m-auto gap-8 items-center justify-center px-10 bg-zinc-200">
          {isLoading ? <h3>Loading...</h3> : result && result.map((object) => {
            return (
              <div className="flex flex-col gap-3">
                <Image ref={imageRef} src={object.url} key={object.index} height={400} width={400} className="rounded-sm" />
                {/* <button
                  onClick={handleSave}
                  className="bg-zinc-700 text-white w-fit m-auto font-bold text-base hover:opacity-70 hover:scale-95 duration-200 rounded-lg tracking-tighter uppercase py-2 px-3"
                >Save Me
                </button> */}
              </div>
            )
